...
async def generate_draft(
    ...
    if not zip_path.exists():
        raise HTTPException(500, "DICOM file not found on disk.")
    ...
    seg_model = get_segmentation_model(
        weights_path=str(Path(settings.MODELS_PATH) / "unet_weights.pth")
    )
    seg_result = seg_model.run(slices, study.body_part or "GENERIC")

    # Save segmentation result
    overlay_path = Path("uploads") / f"overlay_{study.id}.png"
    import base64, io
